...
```{r}
if (file.exists("/Users/jackson/Desktop/Thesis/Data/CES20_Common_OUTPUT_vv.csv")) {
...
data_2 <- data %>% 
  ...
  mutate(
    ...
    over_reported_vote = ifelse(intend_to_vote == 1 , 1, 0),
...
summarize_overreporters <- function(nonvoters, category) {
  nonvoters %>%
    group_by({{ category }}) %>% 
    summarize(count = n(),
              overreport = sum(over_reported_vote, na.rm = TRUE),
              prop = overreport / count)
}

# Use the custom function for each category
...
model_1 <- glm(over_reported_vote ~ as.factor(is_politically_engaged) 
                        +
                          as.factor(education) + 
                 as.factor(income) +
                 Black +
