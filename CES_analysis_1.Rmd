...
if (file.exists("/Users/jackson/Desktop/Thesis/Data/CES20_Common_OUTPUT_vv.csv")) { ## for me :)
  ...
  data <- read_csv("https://dataverse.harvard.edu/api/access/datafile/4949558")
} 
```

### Transform data - # Relevant variables subsetted from CES 2020 dataset, and re-coded/ to mirror variable specification from Ansolabehere and Hersh 2012. 
...
#prop.table(
  table(nonvoters$over_reported_intention)
#)
...
```{r}
### Create user engagement categories; either active/passive or active/moderate/passive
...
is_politically_engaged_proportions <- nonvoters %>% 
  group_by(is_politically_engaged, over_reported_intention) %>% 
  ...
  ggplot(aes(x = is_politically_engaged, y = proportion, fill = over_reported_intention)) +
...
user_engagement_proportions <- nonvoters %>% 
  group_by(user_engagement_category, over_reported_intention) %>% 
  summarize(count = n(), .groups = "drop") %>% 
  mutate(over_reported_intention = ifelse(over_reported_intention == 1, "Yes", "No")) %>% 
  mutate(over_reported_intention = factor(over_reported_intention, levels = c("No", "Yes"))) %>%
  ...
  ggplot(aes(x = user_engagement_category, y = proportion, fill = over_reported_intention)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_grey(name = "Over-reported vote") +
  xlab("is politically engaged?") +
  ylab("Proportion") +
...
sme_proportions <- nonvoters %>% 
  group_by(social_media_engagement, over_reported_intention) %>% 
  summarize(count = n(), .groups = "drop") %>%
  mutate(over_reported_intention = ifelse(over_reported_intention == 1, "Yes", "No")) %>% 
  mutate(over_reported_intention = factor(over_reported_intention, levels = c("No", "Yes"))) %>%
  ...
  ggplot(aes(x = social_media_engagement, y = proportion, fill = over_reported_intention)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_grey(name = "Over-Reported Vote") +
...
ideo_proportions <- nonvoters %>% 
  group_by(ideo5, over_reported_intention) %>% 
  summarize(count = n(), .groups = "drop") %>%
  mutate(over_reported_intention = ifelse(over_reported_intention == 1, "Yes", "No")) %>% 
  mutate(over_reported_intention = factor(over_reported_intention, levels = c("No", "Yes"))) %>%
  group_by(ideo5) %>% 
  ...
  ggplot(aes(x = ideo5, y = proportion, fill = over_reported_intention)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_grey(name = "Over-Reported Vote") +
  xlab("Ideology") +
  ylab("Proportion") +
...
model_1_cts<- glm(over_reported_intention ~ as.factor(is_politically_engaged) 
                        +
                          education +
                 faminc_new +
                 Black +
...
model_2_cts <- glm(over_reported_intention ~ social_media_engagement + 
                 education +
                 faminc_new +
                 Black +
                 other_non_White +
...
model_3a <- glm(over_reported_intention ~ as.factor(saw_story) + as.factor(followed_political_event) + as.factor(posted_story_link) + as.factor(posted_comment) + as.factor(forwarded_story)
               ,  
               data = nonvoters, family = binomial
             ) 

model_3_cts <- glm(over_reported_intention ~ 
...
model_posted <- glm(over_reported_intention ~ 
...
model_4a <- glm(over_reported_intention ~ as.factor(passive_user), data = nonvoters_engaged, family=binomial
                
                )

model_4_cts <- glm(over_reported_intention ~ as.factor(passive_user) + 
                 education +
...
```{r}
model_6 <- glm(over_reported_intention ~ as.factor(social_media_engagement) * as.factor(ideo5) +  #<--------------
                   as.factor(education) + 
                 as.factor(income) +
                 Black +
                 other_non_White + 
...
model_6a <- glm(over_reported_intention ~ as.factor(social_media_engagement) * as.factor(ideo5) 
               ,  
                 data = nonvoters, family = binomial
               )
...
model_6_cts <- glm(over_reported_intention ~ social_media_engagement * ideo5 +
                      education +
                 faminc_new + 
                 #White + 
                 Black + 
...
model_4_robust_cts <- glm(over_reported_intention ~ as.factor(user_engagement_category) #binary
                        +
...
model_4a_robust <- glm(over_reported_intention ~ as.factor(user_engagement_category) #binary
                 ,
                 data = nonvoters_engaged, family = binomial
                #, weights=commonweight
                )

model_4_robust <- glm(over_reported_intention ~ as.factor(user_engagement_category) #binary
                      +  as.factor(education) + 
                 as.factor(income) +
                 Black +
                 other_non_White + 
...
```{r}
model_5_robust <- glm(over_reported_intention ~ as.factor(is_politically_engaged) * as.factor(ideo5) + 
                 as.factor(education) + 
                 as.factor(income) +
                 Black +
                 other_non_White + 
...
model_5a_robust <- glm(over_reported_intention ~ as.factor(is_politically_engaged) * as.factor(ideo5) , 
                 data = nonvoters, family = binomial
...
models_no_covariates <- list(model_1a, model_2a, model_3a, model_4a, model_4a_robust, model_6a)
#models_no_covariates_unweighted <- list(model_1a_u, model_2a_u, model_3a_u, model_4a_u, model_4a_u, model_6a_u)
...
#Replicating logit model from Ansolabehere and Hersh 2012, but with vote intention
...
#Exact replic of model, only using 2020 data instead of 2008
model_robust2 <- glm(over_reported_vote ~
                 education +
